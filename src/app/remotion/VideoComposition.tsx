...
export const VideoComposition: React.FC<VideoCompositionProps> = ({
  ...
  // Add video sequences rendering function
  const renderVideoSequences = () => {
    if (!videoSequences || videoSequences.length === 0) {
      // If no sequences but a single videoUrl is provided (legacy support)
      if (videoUrl) {
        return (
          <Sequence from={0}>
            <Video
    ...
    return (
      <>
        {videoSequences.map((sequence, index) => (
          <Sequence 
            key={`video-${index}`} 
  ...
  // Modified renderImageSequence function to use startFrame and durationInFrames if provided
  const renderImageSequence = () => {
    if (images.length === 0) return null;
    
    // Default seconds per image if not specified
    const defaultSecondsPerImage = 5;
    const defaultFramesPerImage = defaultSecondsPerImage * fps;
    
    return (
      <>
        {images.map((item, index) => {
          // Use provided values or calculate defaults
          const startFrame = item.startFrame !== undefined 
            ? item.startFrame 
            : index * defaultFramesPerImage;
            
          const duration = item.durationInFrames !== undefined 
            ? item.durationInFrames 
            : defaultFramesPerImage;
          
          // Calculate animation progress within this image's timeframe
          const frameWithinImage = Math.max(0, frame - startFrame);
          const progress = Math.min(1, frameWithinImage / duration);
          ...
          // Apply animations
          const animatedStyle = imageAnimation !== 'none'
            ? applyImageAnimation(index, progress, baseStyle)
            : baseStyle;
          
          return (
            <Sequence key={`image-${index}`} from={startFrame} durationInFrames={duration}>
              <AbsoluteFill style={{ justifyContent: 'center', alignItems: 'center' }}>
                <img
                  src={item.imageUrl}
                  style={animatedStyle}
                  crossOrigin="anonymous"
                  alt={item.contextText || `Image ${index + 1}`}
                />
              </AbsoluteFill>
            </Sequence>
          );
